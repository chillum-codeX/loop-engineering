"""
One-shot baseline - simple direct prompting without iteration.
"""
...
class OneShotBaseline:
    ...
    async def solve(self, task_input: Dict[str, Any]) -> Dict[str, Any]:
        """Solve task with one-shot prompting."""
        start_time = time.time()

        # Extract task info
        if "question" in task_input:
            prompt = task_input["question"]
        elif "problem" in task_input:
            prompt = task_input["problem"]
        elif "goal" in task_input:
            prompt = task_input["goal"]
        else:
